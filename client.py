...
class Client:
    ...
    def __init__(self, ip_addr, port, filename):
      ...
      #  print(codecs.decode(tracker_response, 'base64'))
     #   print(codecs.decode(tracker_response, 'base64').decode('utf-8'))
        ...
        if(status[1] != "200"):
           ...
           # self.response = tracker_response
            self.handle_tracker_response(tracker_response)
    ...
    def handle_tracker_response(self, tracker_response):
        # decoded_response = tracker_response.decode('utf-8') 
        # split_decoded = decoded_response.split('\r\n\r\n')
        split_decoded = tracker_response.split(b'\r\n\r\n')
     #   split_decoded[1] = split_decoded[1].replace("\\\\", "\\")
   

        # for x in xrange(len(split_decode[1] - 1):
        #     if split_decode[1][x] == "/" and split_deoce[1][x+1] = "/":
        #         split_decode[1] = split_decode

     #   print(split_decoded[1].codecs.decode)
     #   response_dict = decode(split_decoded[1])
        #response_dict = codecs.decode(split_decoded[1], 'unicode_escape')
        response_dict = decode(split_decoded[1])
